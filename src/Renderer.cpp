...
void Renderer::drawLampLight() {
    ...
    for (int y = H - 1; y >= 0; --y) {
        ...
        float a = 32.0f * k * falloff;         // alpha contribution
...
void Renderer::drawMolecule(const Molecule& m, const LavaLamp& lamp) {
    ...
    // Core intensity (stronger at center)
    ...
    // Palette-based base color (pseudo-random & stable per position)
...
void Renderer::clear() {
    // Optional extra clear (kept for API symmetry; background already clears)
    SDL_SetRenderDrawColor(renderer, 8, 10, 14, 255);
    SDL_RenderClear(renderer);
}
